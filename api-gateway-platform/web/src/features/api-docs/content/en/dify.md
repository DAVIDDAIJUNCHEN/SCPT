...
> Audience: teachers who want to build course Agents / workflow apps with Dify and have those Agents call XingYu models.
> After reading you can: configure XingYu as a model provider in Dify and get your first course Agent running.
...
## 1. How Dify and XingYu Relate

```
Your course Agent (built in Dify)
        │ calls
        ▼
XingYu platform (https://ai-platform.sptc.edu.cn/v1) ── auth token ──> model pool (DS-V4.1-Flash, etc.)
```

Dify handles **application orchestration** (prompts, knowledge bases, workflows); XingYu handles **model inference**. The two connect through the OpenAI-compatible API — Dify treats XingYu as a "custom model provider".

**Who it's for**: course teachers who need to give students fixed-flow exercises (e.g. an "English email polisher" or a "code-debugging Bot"). If you only need chatting without orchestration, the XingYu Chat web app is enough.
...
3. One XingYu token (see [Quickstart](/docs/quickstart) for how to get it; it looks like `sk-xxxxxxxx`).
...
| Field | Value |
|---|---|
| API Key | Your XingYu token `sk-xxxxxxxx` |
| API endpoint URL | `https://ai-platform.sptc.edu.cn/v1` (**must end with /v1**) |

Dify validates the credentials immediately when you save. **If you get an SSL / certificate error here, don't blame the token yet** — jump to Section 4 to handle the self-signed certificate issue (the #1 pitfall when connecting Dify to XingYu), then come back.
...
|---|---|
| Model type | LLM |
| Model name | `DeepSeek-V4.1-Flash` (must exactly match the XingYu model name, case-sensitive) |
...
# ① Download the XingYu gateway certificate (-k is needed only for this download, before the cert is trusted)
...
| Credential validation fails with 401 | Token wrong or disabled | Check the token in the XingYu console |
...
- **Knowledge base (RAG) pairing**: when uploading course materials to a Dify knowledge base, the embedding model for retrieval can also point to XingYu's `bge-m3` (same configuration flow; set model type to "Text embedding", see [Embedding & RAG](/docs/embedding)).
- **Billing**: usage generated by Dify apps is charged to your XingYu token. With many users the quota drains fast — keep an eye on the usage stats in the console (billing details in the [Appendix](/docs/faq)).
- **When giving it to students**: the published app link is hosted by Dify, and students never need a XingYu token — the token is used only once when you configure the provider. Do not leak it.
