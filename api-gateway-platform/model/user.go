...
type User struct {
	...
	Phone            string                     `json:"phone" gorm:"type:varchar(20);uniqueIndex;column:phone" validate:"omitempty,min=11,max=11"` // AlloMax: 手机号
	PhoneVerified    bool                       `json:"phone_verified" gorm:"type:bool;default:false;column:phone_verified"`                    // AlloMax: 手机号已验证
...
// GetUserByPhone AlloMax: 按手机号查询用户（未绑定时返回 ErrPhoneNotBound）
func GetUserByPhone(phone string, selectAll bool) (*User, error) {
	...
	if selectAll {
		...
		err = DB.Omit("password", "access_token").Where("phone = ?", phone).First(&user).Error
	}
	if err != nil {
		return nil, err
	}
