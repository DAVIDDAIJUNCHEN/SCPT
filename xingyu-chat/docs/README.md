...
| Stage 4 | 商业化（错峰定价、分层配额、计费对账、并发分级、user_id 三重隔离） | 与 S2/S4 交错 | 🟡 4.2 已上线（09-21）；4.1/4.3 **暂缓**（09-22 拍板：不收费阶段无账本需求，待收费决策后启动） | — |
...
**第二批 · 计费三项（2026-09-22 拍板：4.1/4.3 暂缓，待收费决策后启动）**

| # | 任务 | 要点 | 状态 |
|---|---|---|---|
| 4.1 | 赠金/充值双账户 | `users.GrantedQuota` 拆两个账本，扣减有先后（先赠金后充值），赠金设有效期 | ⏸ **暂缓（2026-09-22 拍板）**：不收费阶段无充值账本需求，待收费决策后启动；届时先评估 new-api 上游充值体系再决定自研 |
...
| 4.3 | Chat 月度赠金自动刷新 | 按自然月重置 svc-chat 额度，替代人工批配额。**原依赖 4.1，随 4.1 一并暂缓**；如人工重置成为负担，先上 cron 定时重置 GrantedQuota 的最小实现（半天，不动数据结构） | ⏸ **暂缓（随 4.1）** |
...
- **第二批（计费三项）**：✅ 4.2 已上线；4.1/4.3 暂缓（09-22 拍板，不收费阶段无账本需求，待收费决策后启动；届时先评估 new-api 上游充值体系）
